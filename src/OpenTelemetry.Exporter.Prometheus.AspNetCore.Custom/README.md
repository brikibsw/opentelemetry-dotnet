...
1. create git tag with command `git tag -a coreunstable-{your version} -m "message"`
`coreunstable-` prefix is required
if you are releasing alpha version, add `-alpha` suffix, if beta - `-beta`, if rc - `-rc`

2. do build of `OpenTelemetry.Exporter.Prometheus.AspNetCore.TraceParts` in Release mode
3. manually upload nuget package to TP nuget repo
