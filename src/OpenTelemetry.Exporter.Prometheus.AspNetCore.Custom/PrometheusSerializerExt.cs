...
internal static partial class PrometheusSerializer
{
    ...
    public static int WriteMetric(byte[] buffer, int cursor, Metric metric, PrometheusMetric prometheusMetric, Resource resource, bool openMetricsRequested = false)
    {
        ...
        if (!metric.MetricType.IsHistogram())
        {
            foreach (ref readonly var metricPoint in metric.GetMetricPoints())
            {
                ...
                cursor = WriteTags(buffer, cursor, metric, resource, metricPoint.Tags);
        ...
        else
        {
            foreach (ref readonly var metricPoint in metric.GetMetricPoints())
            {
                ...
                foreach (var histogramMeasurement in metricPoint.GetHistogramBuckets())
                {
                    ...
                    cursor = WriteTags(buffer, cursor, metric, resource, tags, writeEnclosingBraces: false);
